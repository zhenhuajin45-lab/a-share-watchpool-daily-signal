...
def main() -> int:
    ...
    if args.kaipanla:
        ...
        sources["kaipanla"] = {"pages": pages, "ready": len(pages) == 4 and all(row["status"] == "OK" for row in pages)}
    if args.external:
    ...
    result = {"schema_version": "premarket_source_health_v1", "sources": sources, "ready": all(row.get("ready", row.get("publishable", False)) is True for row in sources.values())}
